...
const CategoriesSidebar = async () => {
  ...
  try {
    // Fetch product types
    // Note: The API returns an array directly for /product-types, but let's check if we need to wrap it or not.
    // Based on productTypeController.ts: res.json(productTypes) -> returns array directly.
    // However, we used fetchData<ProductTypesResponse> which expects an object.
    // Let's assume fetchData handles the response. If the API returns an array, fetchData generic might need adjustment or we cast.
    // Actually, looking at productTypeController.ts, getProductTypes returns `res.json(productTypes)`.
    // So it returns ProductType[].
    const productTypesData = await fetchData<ProductType[]>("/product-types", {
      next: { revalidate: 3600 },
    });
    productTypes = productTypesData.slice(0, 10);

    // Fetch root categories
    const rootCategoriesData = await fetchData<CategoriesResponse>(
      "/categories?parent=null&perPage=15",
      { next: { revalidate: 3600 } },
    );
    rootCategories = rootCategoriesData.categories;
