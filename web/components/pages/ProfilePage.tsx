...
import type { Address as AddressType, User as UserType } from "@/lib/types";
    ...
    if (data.name !== authUser?.name || avatarPreview) {
      ...
      try {
        ...
        if (response.success && response.data) {
          ...
          toast.error(
            response.error?.message || "Failed to update profile details",
          );
        }
      } catch (error) {
      ...
      try {
        ...
        if (response.success) {
          ...
          if (
            ...
            setIsLoading(false);
            return; // Don't close sidebar if password failed
          }
          toast.error(msg);
        }
