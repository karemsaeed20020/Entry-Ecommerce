...
const ChatWidget: React.FC<ChatWidgetProps> = ({ 
  ...
  const initChat = async () => {
    ...
    try {
      ...
      const convRes = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/chat/conversation`, {
        method: "POST",
        headers: {
      ...
      if (convData.success) {
        ...
        // 2. Fetch Messages
        const msgRes = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/chat/messages/${cId}`, {
      ...
      {isOpen && (
        <div 
          className={cn(
            ...
            isMinimized ? "h-16 w-64" : "h-[500px] w-[380px] max-w-[calc(100vw-48px)]"
          )}
        >
          {/* Header */}
          <div className="bg-[#1a1a2c] p-4 flex items-center justify-between text-white shrink-0">
            <div className="flex items-center gap-3">
              <div className="h-8 w-8 rounded-full bg-white/10 overflow-hidden flex items-center justify-center border border-white/10">
                {sellerAvatar ? (
                  <Image src={sellerAvatar} alt="avatar" width={32} height={32} className="object-cover" />
                ) : (
                  <User className="h-4 w-4 text-white/40" />
                )}
              </div>
              <div>
                <p className="text-xs font-black truncate max-w-[150px]">{sellerName || "Seller"}</p>
                ...
                {loading ? (
                  ...
                  <div className="flex flex-col items-center justify-center h-full text-center p-6 opacity-30">
                    ...
                    <p className="text-[10px] font-black uppercase tracking-widest leading-relaxed">
                      Start a conversation with<br />{sellerName || "this seller"}
                    </p>
                  </div>
                ) : (
