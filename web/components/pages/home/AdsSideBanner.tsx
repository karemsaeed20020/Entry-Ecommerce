    ...
    const fetchBannerData = async () => {
      try {
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000"}/api/ads-banners?bannerType=offer&isActive=true&perPage=1`,
        );
        const data = await res.json();

        if (data.adsBanners && data.adsBanners.length > 0) {
          setBanner(data.adsBanners[0]);
        }
      } catch (error) {
        if (process.env.NODE_ENV === "development") {
          console.error("Failed to fetch ads banner for side:", error);
        }
      } finally {
        setLoading(false);
      }
