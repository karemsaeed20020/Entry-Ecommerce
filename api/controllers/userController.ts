...
const updateUser: RequestHandler = asyncHandler(async (req: RequestWithBody<UpdateUserBody>, res) => {
  ...
  const result = await uploadService.replaceImage(
    ...
    {
      folder: "users",
      provider: "cloudinary", // Explicitly set this
      enableFallback: false,   // Stop it from trying S3 if Cloudinary fails
      originalName: `avatar_${user.email.replace("@", "_").replace(".", "_")}.jpg`,
    }
  );
