    ...
    const verifyEmail = async () => {
      ...
      try {
        ...
        if (response.success) {
          ...
          // Refresh user data in store to reflect verification status immediately
          await verifyAuth();
          ...
          setStatus("error");
          setMessage(typeof response.error === "string" ? response.error : response.error?.message || "Verification failed. The link may be invalid or expired.");
        }
      } catch (error) {
        setStatus("error");
