...
import { SuccessPageSkeleton } from "../skeleton";
...
const SuccessPageContent = ({
  ...
  // Stripe payment status update
  useEffect(() => {
    if (!order || !sessionId || statusUpdated) return;
    if (!needsPaymentUpdate(order, sessionId)) {
      if (order.paymentStatus === "paid") setStatusUpdated(true);
      return;
    }
    ...
    const update = async () => {
      try {
        const result = await handlePaymentSuccess(orderId, sessionId, token);
                ...
                order.payment_info.stripe && (
                  <>
                    {order.payment_info.stripe.paymentMethodType && (
                      <PaymentRow
                        label="Method"
                        value={order.payment_info.stripe.paymentMethodType}
                      />
                    )}
                    {order.payment_info.stripe.cardBrand && (
                      <PaymentRow
                        label="Card"
                        value={`${order.payment_info.stripe.cardBrand.toUpperCase()}${
                ...
                order.payment_info.sslcommerz && (
                  <>
                    ...
                    {order.payment_info.sslcommerz.cardBrand && (
                      <PaymentRow
                        label="Card"
                        value={order.payment_info.sslcommerz.cardBrand}
                      />
                    )}
                    {order.payment_info.sslcommerz.transactionId && (
                      <PaymentRow
                        label="Transaction ID"
                        value={order.payment_info.sslcommerz.transactionId}
                      />
                    )}
                  </>
                )}
